...
def delete_log_and_text_files():
    files_to_delete = glob.glob(os.path.join(scripts_dir, "*.log")) + glob.glob(os.path.join(scripts_dir, "*.txt"))
    for file_path in files_to_delete:
        try:
            os.remove(file_path)
            print(f"[{current_timestamp()}] Deleted file: {file_path}")
        except Exception as e:
            print(f"[{current_timestamp()}] Failed to delete {file_path}: {e}")
